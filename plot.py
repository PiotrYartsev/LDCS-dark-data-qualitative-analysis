...
for row in con.execute('SELECT name FROM sqlite_master WHERE type = "table" ORDER BY name').fetchall():
    if row[0] == 'sqlite_sequence':
        ...
        plt.plot(time_1_list,postion_duplicate,".",label="Early duplicate", markersize=7,color="black")
        plt.plot(time_2_list,postion_duplicate,".",label="Later duplicate", markersize=7,color="red")
        plt.plot(time_no_problem,position_regular,".",label="Not a duplicate", markersize=4,color="white")
        #plt.grid(linestyle='--',)
        plt.title("Submission time of file for early duplicate, late duplicate and not a duplicate file",fontsize=20)
