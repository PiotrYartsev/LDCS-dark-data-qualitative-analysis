...
for row in con.execute('SELECT name FROM sqlite_master WHERE type = "table" ORDER BY name').fetchall():
    if row[0] == 'sqlite_sequence':
        ...
        data1 = con.execute("SELECT file_number FROM {} WHERE duplicate is not NULL".format(row[0])).fetchall()
        for rows1 in data1:
            filenumber=rows1[0]
            data = con.execute("SELECT ComputingElement,duplicate FROM {} WHERE duplicate is {}".format(row[0],filenumber)).fetchall()
            print(len(data))
            for rows in data:
                print(rows)
                location=rows[0]
